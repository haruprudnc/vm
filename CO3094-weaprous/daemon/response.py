...
#                "Cookie": "{}".format(reqhdr.get("Cookie", "sessionid=xyz789")), #dummy cooki
	...
	# self.auth = ...
        ...
        for key, value in self.headers.items():
            if value and key != 'Content-Type':  # Content-Type already handled
                header_lines.append("{}: {}\r\n".format(key, value))
        
        header_text = status_line + "".join(header_lines) + "\r\n"
    ...
    def build_json_response(self, request, data, status_code=200, reason="OK"):
        """
        Builds a JSON HTTP response.
        
        :param request: The :class:`Request <Request>` object.
        :param data: Dictionary or object to serialize as JSON.
        ...
        try:
            json_str = json.dumps(data) if isinstance(data, dict) else json.dumps(data)
            self._content = json_str.encode('utf-8')
        except (TypeError, ValueError) as e:
            print("[Response] Error serializing JSON: {}".format(e))
            error_data = {"error": "Failed to serialize response", "message": str(e)}
            self._content = json.dumps(error_data).encode('utf-8')
            self.status_code = 500
            self.reason = "Internal Server Error"
        
        response_header = self.build_response_header(request)
        return response_header + self._content
    ...
    def build_response(self, request):
        ...
        #^^--------------------------------------------------------------------^^#
        print("[response.py-build_response] RAW response content: \n{}\n\n^^====================================================================^^\n\n\n".format((self._header + self._content).decode('utf-8').replace('\r\n', '\n')))
        #^^====================================================================^^#
